            ...
            <div class="card-body">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" v-model="selectedProduct" @change="productSelected">
                        <option selected disabled>Lutfen Bir Urun Seciniz..</option>
                        <option :disabled="product.count == 0" v-for="product in getProducts" :value="product.key"> {{ product.title }} </option>
                    </select>
                </div>
...
export default {
    ...
    computed : {
        ...
        saveEnabled() {
            if (
                ...
                this.product_count > 0 && this.product_count <= this.product.count
            ) {
                return false;
            } else {
                return true;
    ...
    if (
      (this.selectedProduct !== null || this.product_count > 0) && !this.saveButtonClicked
    ) {
      if (
        confirm(
          "Kaydedilmemis deyisiklikler var. Yinede cikmak istiyormusunuz ?"
        )
      ) {
        next();
      } else {
